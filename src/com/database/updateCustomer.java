...
public class updateCustomer {
	...
	public static void updateCustomerAccount() throws SQLException {
		...
		if(!addData.checkSSNID(customerSSID)) {
			...
			Customer newData = new Customer(oldData.getCustomerSSNID(),newName, 
					newAge, newAddress, newCity, newState);
			
			if (updateCus(newData)) {
				System.out.println("Customer update initiated "
						+ "successfully");
			}
			else {
		...
		try {
			...
			while(resultSet.next()) {
				...
				if(SSNID == cSSNID) {
					...
					temp = new Customer(cid, cSSID,cName, cAge
